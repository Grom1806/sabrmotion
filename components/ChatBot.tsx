...
function renderInline(text: string, keyPrefix: string) {
  ...
  return parts.map((part, i) => {
    if (part.startsWith('**') && part.endsWith('**')) {
      return (
        <strong key={`${keyPrefix}-${i}`} style={{ color: 'var(--text)', fontWeight: 700 }}>
          {part.slice(2, -2)}
        </strong>
      )
    }
      ...
      {lines.map((raw, i) => {
        ...
        if (bullet) {
          return (
            <div
              key={i}
              style={{ display: 'flex', gap: '0.5rem', alignItems: 'baseline', margin: '0.05rem 0' }}
            >
...
const errorText: Record<Lang, string> = {
  ru: '⚠️ Не удалось ответить. Напишите напрямую: @username',
  en: "⚠️ Couldn't respond. Message me directly: @username",
  de: '⚠️ Keine Antwort möglich. Schreiben Sie direkt: @username',
}
...
export default function ChatBot() {
  ...
  const runStep = useCallback(
    async (stepKey: string, currentLang: Lang) => {
  ...
  return (
    <>
            ...
            >
                  ...
                  >
                    <div
                      style={{
                        ...
                        fontFamily: 'var(--font-inter)',
                        fontSize: '0.83rem',
                        color: msg.from === 'user' ? 'var(--text)' : 'var(--text-sec)',
                        lineHeight: 1.6,
                      }}
                    >
                      {msg.from === 'bot'
              ...
              {typing && (
                ...
                >
                  ...
                  >
                    <span style={{ display: 'flex', gap: '4px', alignItems: 'center', height: '8px' }}>
                      {[0, 1, 2].map(j => (
                        <motion.span
                          key={j}
                          animate={{ y: [0, -4, 0] }}
                          transition={{ duration: 0.6, repeat: Infinity, delay: j * 0.14 }}
                          style={{
                            width: '5px',
                            height: '5px',
                            borderRadius: '50%',
            ...
            >
              ...
              <motion.button
                ...
                style={{
                  ...
                  height: '36px',
                  borderRadius: '50%',
                  background: input.trim() && !loading ? 'var(--accent)' : 'rgba(255,255,255,0.07)',
                  border: 'none',
        ...
        {showTeaser && (
          <motion.div
            ...
            style={{
              ...
              boxShadow: '0 8px 28px rgba(0,0,0,0.45), 0 0 18px rgba(74,222,128,0.12)',
            }}
          >
            <button
      ...
      <motion.button
        ...
        transition={
          showTeaser ? { duration: 2, repeat: Infinity, ease: 'easeInOut' } : { duration: 0.3 }
        }
        ...
        <AnimatePresence mode='wait'>
          {open ? (
            <motion.span
              ...
              style={{ fontSize: '1.1rem', color: '#0a0a0a', lineHeight: 1, fontWeight: 700 }}
            >
              ✕
            </motion.span>
          ) : (
